...
class CowCostController extends Controller
{
    ...
    public function show(Request $request, $cowId)
    {
        try {
            ...
            // Column names in financial_records: transaction_date (or record_date if fallback)
            $directRecords = DB::table('financial_records')
                ->where('related_cow_id', $cowId)
                ->select('financial_record_id', 'farm_id', 'title', 'trans_type', 'category', 'amount', 'transaction_date', 'notes', 'created_at')
            ...
            \Illuminate\Support\Facades\Log::error("[CowCostController] Error for cow {$cowId}: " . $e->getMessage() . "\n" . $e->getTraceAsString());
            return response()->json([
                'message' => 'Failed to calculate cow costs',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
