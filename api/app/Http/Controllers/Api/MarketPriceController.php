...
1. การค้นหาส่วนราคาโคเนื้อ:
   - ค้นหาส่วนหรือตารางราคาที่เกี่ยวกับ "โคเนื้อ" หรือ "โคเนื้อและกระบือ" หรือสัตว์ประเภทโค/วัว โดยไม่ต้องยึดติดกับรูปแบบเลย์เอาต์ (ไม่ว่าจะแสดงเป็นตาราง, การ์ด, คอลัมน์ หรือรายการ)
2. การเลือกงวดราคาล่าสุด (ไม่ล็อกตำแหน่งหรือลำดับคอลัมน์):
   - ตรวจสอบหัวตารางหรือหัวข้อระบุวันที่ของราคา หากมีหลายคอลัมน์ราคาหรือหลายงวด (เช่น มีราคาเปรียบเทียบสัปดาห์ก่อนหน้า กับสัปดาห์ปัจจุบัน/สัปดาห์นี้): ให้เลือกอ่านราคาจากคอลัมน์หรืองวดที่มี "วันที่ล่าสุดที่สุด (Most recent date)" เสมอ โดยพิจารณาจากวันที่ที่ระบุเป็นหลัก ไม่ต้องยึดติดว่าต้องเป็นคอลัมน์ลำดับที่เท่าใด
   - หากมีคอลัมน์ราคาเพียงคอลัมน์เดียว ให้ใช้คอลัมน์ราคานั้นได้ทันที
3. วันที่และปี พ.ศ./ค.ศ. (effective_date & report_date_text):
   - ให้นำวันที่ของงวดราคาล่าสุดดังกล่าว (จากหัวคอลัมน์ราคาล่าสุด หรือจากวันที่รายงานในภาพ เช่น 'ราคา ณ วันที่ 16 ส.ค. 69' หรือ '16 สิงหาคม 2569') มาใช้เป็น effective_date โดยแปลงปี พ.ศ. เป็น ค.ศ. ในรูปแบบ YYYY-MM-DD เสมอ (เช่น พ.ศ. 2569 / '69 -> 2026 จึงได้ '2026-08-16')
   - นำข้อความวันที่เดิมที่พบมาใส่ใน report_date_text (เช่น '16 สิงหาคม 2569')
4. การสกัดราคาและสายพันธุ์โคเนื้อ (cattle_prices):
   - สกัดตัวเลขราคาและชื่อสายพันธุ์เฉพาะ 'โคเนื้อ / วัว' เท่านั้น (เช่น ลูกผสมยุโรป, ลูกผสมบราห์มัน, โคพื้นเมือง) พร้อมพิกัดน้ำหนักถ้ามี จากงวดราคาล่าสุดนั้น
   - *** สำคัญมาก: ไม่ต้องเอา 'กระบือ' หรือ 'กระบือเนื้อ' (ควาย) มาโดยเด็ดขาด ให้ตัดออก ไม่ต้องรวมเข้ามาในรายการ ***
ตัวอย่างรูปแบบ JSON:
{
  "is_dld_report": true,
  "title": "หัวข้อรายงาน เช่น ราคาเฉลี่ยสินค้าปศุสัตว์ที่เกษตรกรขายได้ สัปดาห์ที่ 2 เดือน กันยายน 2569",
  "report_date_text": "วันที่ของราคาล่าสุด เช่น 16 สิงหาคม 2569",
  "effective_date": "YYYY-MM-DD ของวันที่ราคาล่าสุด เช่น 2026-08-16",
  "cattle_prices": [
    {
        ...
        if (!empty($aiParsed['cattle_prices']) && is_array($aiParsed['cattle_prices'])) {
            foreach ($aiParsed['cattle_prices'] as $item) {
                ...
                $extractedItems[] = [
                    ...
                    'note' => "{$reportTitle}" . ($reportDateText ? " (ราคา ณ วันที่ {$reportDateText})" : ""),
                ];
            }
        }
